    ...
    <table class="table table-hover align-middle bg-white border text-secondary">
        ...
        <tbody>
            ...
            <tr>
                <td>
                    @if ($user->avatar)
                    <img src="{{ $user->avatar }}" alt="{{ $user->nane }}" class="rounded-circle d-block mx-auto avatar-md">
                    @else
                    <i class="fa-solid fa-circle-user d-block mx-auto icon-md"></i>
                    @endif
                </td>
                <td>
                        ...
                        <div class="dropdown-menu">
                            @if ($user->trashed())
                                <button class="dropdown-item" data-bs-toggle="modal" data-bs-target="#activate-user-{{ $user->id }}">
                                    <i class="fa-solid fa-user fa-user-check"></i> Activate {{ $user->name }}
                                </button>
                                
                            @else
                                <button class="dropdown-item text-danger" data-bs-toggle="modal" data-bs-target="#deactivate-user-{{ $user->id }}">
                                    <i class="fa-solid fa-user fa-user-slash"></i> Deactivate {{ $user->name }}
                                </button>
                            @endif
                        </div>
                    </div>
